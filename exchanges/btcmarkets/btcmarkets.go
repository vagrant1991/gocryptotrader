...
func (b *BTCMarkets) SendAuthenticatedRequest(reqType, path string, data interface{}, result interface{}) (err error) {
	...
	hmac := common.GetHMAC(common.HashSHA512, []byte(request), []byte(b.API.Credentials.Secret))

	if b.Verbose {
		log.Printf("Sending %s request to URL %s with params %s\n", reqType, b.API.Endpoints.URL+path, request)
	}
	...
	return b.SendPayload(reqType, b.API.Endpoints.URL+path, headers, bytes.NewBuffer(payload), result, true, b.Verbose)
}
